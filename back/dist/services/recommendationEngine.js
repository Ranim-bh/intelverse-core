...
const DEFAULT_LEAD_SCORING_ROWS = [
...
const resolveTier = (score) => {
    if (score <= 39)
        return { tier: "Solo", nb_rooms: 1 };
    if (score <= 69)
        return { tier: "Duo", nb_rooms: 2 };
    if (score <= 89)
...
const resolveTables = async () => {
  ...
  `);
    ...
    if (!guestTable)
        throw new Error("Missing guest table (expected guest or guest_kpis)");
    if (!servicesTable)
        throw new Error("Missing services table (expected services or service)");
    if (!roomKpiTable)
        throw new Error("Missing room_kpi table");
    if (!packTable || !packServiceTable)
        throw new Error("Missing pack catalog tables (pack, pack_service)");
...
const getNumericColumnsForTable = async (tableName) => {
...
const buildAliasMetricMap = (guestRaw, roomRaw, roomsVisitedRaw) => ({
...
const computeWeightedAverage = (metricValues, weightByKey, eligibleKeys) => {
...
const pickDeterministicFallbackPack = (score, packs) => {
    if (!packs.length) {
    ...
    if (topRoom) {
        const byTopRoom = packs.find((pack) => parseCsvList(String(pack.service_names ?? "")).some((serviceName) => String(serviceName).trim().toLowerCase() === topRoom));
        if (byTopRoom)
            return byTopRoom;
    }
...
const ensureRecommendedOffersTable = async () => {
    await queryDb(`
...
const ensureLeadScoringWeightsTable = async () => {
    await queryDb(`
...
const ensureUserScoresTable = async () => {
    await queryDb(`
...
const normalizeLeadWeightRows = (rows) => {
    const seen = new Set();
    return rows
        .map((row) => {
        const kpiKey = String(row.kpi_key ?? "").trim();
        if (!kpiKey || seen.has(kpiKey))
            return null;
        seen.add(kpiKey);
...
export const getLeadScoringWeights = async () => {
...
export const saveLeadScoringWeights = async (rows) => {
    await ensureLeadScoringWeightsTable();
    const normalizedRows = normalizeLeadWeightRows(rows.map((row) => ({
        ...row,
        created_at: "",
        updated_at: "",
    })));
...
export const getAvailableRecommendedOfferKpis = async () => {
    const existingTables = await queryDb(`
    SELECT table_name
    FROM information_schema.tables
    WHERE table_schema = 'public'
  ...
  `);
    ...
    const isNumeric = (dataType, udtName) => {
        const normalized = String(dataType ?? "").toLowerCase();
        const udt = String(udtName ?? "").toLowerCase();
        return (normalized === "smallint"
            || normalized === "integer"
    ...
    for (const col of columns.rows) {
        ...
        if (!key || reserved.has(key))
            continue;
        if (!isNumeric(col.data_type, col.udt_name))
            continue;
        if (unique.has(key))
            continue;
...
const computeScore = async (guestId) => {
    ...
    const guestRawResult = await queryDb(`
      SELECT
        ${guestSelectColumns}
      FROM ${guestTable}
    ...
    for (const key of selectedGuestColumns) {
        guestRaw[key] = toNum(guestRawRow[key]);
    }
    const guestMaxRaw = {};
    if (selectedGuestColumns.length) {
        const guestMaxSelect = selectedGuestColumns
            ...
            .join(",\n        ");
        const guestMaxResult = await queryDb(`
        SELECT
          ${guestMaxSelect}
        FROM ${guestTable}
      `);
        ...
        for (const key of selectedGuestColumns) {
            guestMaxRaw[key] = toNum(maxRow[key]);
        }
    }
    const roomRaw = {};
    const roomMaxRaw = {};
    ...
    if (selectedRoomColumns.length) {
        const roomSumSelect = selectedRoomColumns
            ...
            .join(",\n        ");
        const roomRawResult = await queryDb(`
        SELECT
          ${roomSumSelect}
        FROM ${roomKpiTable}
        WHERE user_id = $1
      `, [guestId]);
        ...
        const roomMaxOuterSelect = selectedRoomColumns
            .map((col) => `MAX(${quoteIdentifier(col)})::numeric AS ${quoteIdentifier(col)}`)
            .join(",\n          ");
        const roomMaxResult = await queryDb(`
        SELECT
          ${roomMaxOuterSelect}
        FROM (
          SELECT
      ...
      `);
        ...
        for (const key of selectedRoomColumns) {
            roomMaxRaw[key] = toNum(roomMaxRow[key]);
        }
    }
    const roomsVisitedRawResult = await queryDb(`
    ...
    const roomsVisitedMaxResult = await queryDb(`
...
const callGroqRecommendation = async (score, packs) => {
    ...
    const messages = [
        {
            role: "system",
            content: "You are a recommendation engine. Select ONE pack from the provided list only. Return strict JSON with keys: recommended_pack_code, reason.",
        },
        {
            role: "user",
...
export const recommendForGuest = async (guestId) => {
    ...
    try {
        ...
        if (!selected) {
            selected = packs.find((pack) => n(pack.pack_code) === n(ai.recommended_pack_code));
        }
        if (selected) {
            reason = ai.reason;
        }
...
export const saveRecommendationForGuest = async (guestId, recommendation) => {
    const canonicalRecommendation = await recommendForGuest(guestId);
    const normalizedRecommendation = canonicalRecommendation;
...
export const listRecommendedOffers = async () => {
    await ensureRecommendedOffersTable();
    const result = await queryDb(`
    SELECT
      offer_id::text,
  ...
  `);
    return result.rows
        .map((row) => {
        if (!row.offer_payload || typeof row.offer_payload !== "object")
            return null;
        return {
            offer_id: row.offer_id,
            user_id: row.user_id,
            ...
            offer_payload: row.offer_payload,
            status: row.status,
...
export const updateRecommendedOfferStatus = async (guestId, status) => {
    await ensureRecommendedOffersTable();
    const result = await queryDb(`
      UPDATE recommended_offers
      SET status = $2,
