...
app.get("/api/offers/:user_id", async (req, res) => {
  try {
    ...
    const offer = await getCurrentOfferByUserId(userId);
    if (!offer) {
...
app.patch("/api/offers/:offer_id/status", async (req, res) => {
  try {
    ...
    const updated = await updateOfferStatusById(offerId, status);
    if (!updated) {
      return res.status(404).json({ error: "Offer not found" });
    }
