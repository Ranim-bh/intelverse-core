...
import { normalizeRecommendResponse } from "@/lib/recommendation-client";
...
async function callGeneratePack(userId: string, retryOnParseError = true): Promise<PersonalizedPack> {
  ...
  const canonical = normalizeRecommendResponse(payload);

  if (canonical.recommended_pack) {
    const services = Array.isArray(canonical.recommended_pack.services)
      ? canonical.recommended_pack.services.map((name, index) => ({
        id: String(index + 1),
        name,
        price: 0,
        reason: canonical.recommended_pack?.reason ?? "Recommendation generated by AI.",
      }))
      : [];
    ...
    return {
      name: canonical.recommended_pack.pack_name,
      description: `Tier ${canonical.tier} · Pack code ${canonical.recommended_pack.pack_code}`,
      services,
      total_price: 0,
      kpis_addressed: scoreBreakdown
        ? [scoreBreakdown.top_room, scoreBreakdown.top_room_by_interactions].filter(Boolean)
        : [],
      match_score: `${canonical.score}%`,
      summary: canonical.recommended_pack.reason,
    };
  }
